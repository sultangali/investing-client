...
const Eibd = ({ index, item, chartData, currentTime }) => {
    ...
    const minimize = (number, item, previousValue) => {
        ...
        switch(item) {
            ...
            case 'balanced':
                newValue = previousValue ? previousValue + baseValue * (Math.random() * 0.100 + 0.100) : 0.010;
    ...
    chartData[item.name]?.forEach((itm, index) => {
        ...
        dps['increased'].push({
            ...
            y: parseFloat(minimize(itm?.data?.equilibrium['1'].increased, 'increased', prevIncreased).toFixed(3))
        })
        dps['balanced'].push({
            x: index,
            label: 'balanced: ' + (index + 1),
            y: parseFloat(minimize(itm?.data?.equilibrium['1'].balanced, 'balanced', prevBalanced).toFixed(3))
        })
        dps['decreased'].push({
            x: index,
            label: 'decreased: ' + (index + 1),
            y: parseFloat(minimize(itm?.data?.equilibrium['1'].decreased, 'decreased', prevDecreased).toFixed(3))
        })
    })
