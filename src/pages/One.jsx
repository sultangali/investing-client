...
import TableRowShort from "./TableRowShort.jsx";
import EOF from "./EOF.jsx";
import Eibd from "./Eibd.jsx";
import Oibd from "./Oibd.jsx";
import Fibd from "./Fibd.jsx";
import FRNumber from "./FRNumber.jsx";
import FFRNumber from "./FFRNumber.jsx";
import Last from "./Last.jsx";
import FourValues from "./fourValues.jsx";
import { useTranslation } from 'react-i18next';

const One = ({ item, index, currentTime, currentYear, currentLanguage }) => {
    const isAuth =useSelector(fetches.selectIsAuth);
    const { id } = useParams();
    ...
    const calculateData = (item) => {
        ...
        const getPreviousObservation = (name, year) => {
            ...
            let previousData = getOneData(name, year, false);
            console.log('previousData', getOneData(name, year, false))
            if (previousData) {
        ...
        if (YEAR === 1995 || previousObservation === null) {
            ...
            if (change > 0) {
                ...
                observation1.increased = (Math.abs(change));
                observation1.balanced = 1 - (observation1.increased);
                observation1.decreased = 0;
            } else if (change < 0) {
                observation1.increased = 0;
                observation1.balanced = 1 - Math.abs(change);
                observation1.decreased = Math.abs(change);
            } else {
                observation1.increased = 0;
        ...
        if (YEAR === `1995` || previousObservation === null) {
            ...
            if (change > 0) {
                equilibrium1.increased = Math.abs(change);
                equilibrium1.balanced = (1 - equilibrium1.increased);
                equilibrium1.decreased = 0;
            } else if (change < 0) {
                equilibrium1.increased = 0;
                equilibrium1.balanced = (1 - Math.abs(change));
                equilibrium1.decreased = Math.abs(change);
            } else {
                equilibrium1.increased = 0;
        ...
        if (YEAR === `1995` || previousObservation === null) {
            equilibrium3.increased = Math.abs(1)
            equilibrium3.balanced = Math.abs(0)
            equilibrium3.decreased = 0
        } else {
            const change = (equilibrium3.root - previousObservation) / previousObservation;
            if (change > 0) {
                equilibrium3.increased = Math.abs(change);
                equilibrium3.balanced = 1 - equilibrium3.increased;
                equilibrium3.decreased = 0;
            } else if (change < 0) {
                equilibrium3.increased = 0;
                equilibrium3.balanced = 1 - Math.abs(change);
                equilibrium3.decreased = Math.abs(change);
            } else {
                equilibrium3.increased = 0;
                equilibrium3.balanced = 1;
                equilibrium3.decreased = 0;
            }
        }
    ...
    return (<>
        <Container>
            ...
            <Stack direction="horizontal" gap={3}>
                <div className="p-2"><h1>{hourlyData?.map((item, index) => item?._id && item?.lang[switchLanguage(currentLanguage)])}</h1></div>
            ...
            <Row>
                ...
                <Col>
                    <Table bordered className="data-table">
                        ...
                        <tbody>
                            {hourlyData?.map((item, index) => item?._id && (
                                <TableRowShort
                                    key={item.index}
                                    index={index}
                                    item={item}
                ...
                { !isAuth ? <><div className="text-center" style={{margin: '32px auto'}}><h3> {t('to-login')}</h3> <hr /><Button className="btn btn-primary" href="/login" style={{borderRadius: '1px'}}>{t('signin-title')}</Button></div> </> : <>
                    <Col className="col-4" >
                    <div style={{ border: '1px solid #4361ee' }}>
                        <br />
                        {hourlyData?.map((item, index) => item?._id && (
